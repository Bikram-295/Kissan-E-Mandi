...
const FarmerDashboard = ({ navigation, route }) => {
  ...
  const handleList = async () => {
    console.log("Listing crop with data:", { crop, quantity, recipient });
    if (crop != "" && quantity != "" && recipient != "") {
      try {
        console.log("Existing crops:", crops);
        let match = false;
        crops && crops.forEach(cropobj => {
        ...
        if (!match) {
          console.log("Error: invalid crop name. Not found in:", crops);
        ...
        console.log("Submitting listing for farmer:", currFarmer);
        ...
        console.log("Listing creation successful:", response);
        setOpen(false)
        // Optionally refresh listings here if not using auto-revalidation
      }
      catch (error) {
        console.error("Listing creation failed:", error.response ? error.response.data : error.message);
    ...
    return (
      <Card key={listing.id} style={styles.cards}>
        <Card.Content style={{ flexDirection: "row", justifyContent: "space-between" }}>
          ...
          <Text variant="bodyLarge" style={{ color: "#128100" }}>By MSP : Rs {isNaN(totalMSP) ? 0 : totalMSP}</Text>
        </Card.Content>
        <Card.Content>
          <Text variant='bodyMedium'>Quantity : {listing.quantity}</Text>
          <Text variant='bodySmall'>MSP  : {cropMSP || "N/A"} rupees per kg</Text>
        </Card.Content>
      </Card>
    )
  })
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScrollView style={{}}>
        ...
        <View style={{ padding: 16 , display : selectedList === "Your crops" || selectedList === "Dashboard" ? "flex" : "none"}}>
        ...
        <View style={{ padding: 16, display : selectedList === "Your deals" || selectedList === "Dashboard" ? "flex" : "none" }}>
          <Text variant='headlineSmall' style={{ marginLeft: 8, marginTop: 8 }}>Your deals</Text>
          <View style={{ alignItems: "center", width: "100%", marginTop: 16 }}>
            {displayDoneDeals}
          </View>
        </View>
        <View style={{ padding: 16, display : selectedList === "New/Pending offers" || selectedList === "Dashboard" ? "flex" : "none"}}>
      ...
      <Portal>
        <Modal style={{ alignItems: "center" }} visible={open} contentContainerStyle={containerStyle} onDismiss={() => setOpen(false)}>
          ...
          <Button mode='contained' style={{ width: '65%', marginTop: 8 }} onPress={handleList}>List</Button>
        </Modal>
      </Portal>
